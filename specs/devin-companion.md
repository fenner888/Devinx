...
Place a compact companion in a dedicated transparent track immediately above the session composer. The track is a stable, pointer-transparent foreground overlay at the bottom of the timeline viewport, so the conversation scrolls behind it and it consumes no opaque layout row. It must not push messages into a visible shelf, live inside the composer, intercept touches, cover controls, or paint a background. The scroll content must include enough transparent tail clearance for every response line to scroll fully above the companion and its caption. Session Detail intentionally uses a much smaller companion than Home so timeline content remains primary. Keep its transparent track mounted throughout the session. While a turn is active, render only the short caption **Devin working** directly above the sprite inside the same animated wrapper; the caption moves and stops with Devin instead of remaining centered in the viewport. The caption has no chip, border, card, opaque fill, task detail, or accumulated activity list. An optimistic send or any canonical active-work state begins walking immediately at Devin's current position without jumping to another edge. The underlying semantic activity state may still select an appropriate thinking, editing, testing, reading, executing, or responding animation, but every active state uses the direction-specific walking frames until work settles. When work ends, stop movement at the current position and transition directly to waiting, success, error, sleeping, or another canonical animation. Passive idle, waiting, and sleeping states remove the caption and its caption-height reservation. Sending immediately dismisses the keyboard so the timeline, track, and composer remain visible. While the keyboard is open, both Cloud and Computer session composers expose a 44-point, VoiceOver-labeled Hide Keyboard action beside the mic and Send controls; dragging the conversation interactively dismisses it as well. Either dismissal path must preserve the complete draft. Use the exhaustive canonical status mapping and canonical status label for accessibility. Do not duplicate activity text in the timeline.

Do not render an activity trail, checklist, accumulated status history, or spinner row in the timeline tail. Computer and Cloud transport activity remains an internal animation-state signal only. Never expose its task label, tool inputs/outputs, thoughts, raw paths, identifiers, or private event payloads through the companion UI.
...
- The single active **Devin working** caption, passive-caption suppression, and absence of a timeline activity trail.
