...
Make Devin the centered visual anchor between a compact readiness card and the full-width composer. Use a responsive presentation of roughly 180–220 points so the character remains prominent without forcing controls off small screens. Home and Session Detail use the semantic `canvas` background, which is true black in dark mode and preserves the normal light canvas. On the dark Home screen only, the companion sits over the approved static space artwork in `assets/pets/devin/home-companion-stage.jpg`. Crop the artwork responsively so its blue horizon and circular floor light sit beneath Devin while its black outer field fades into the true-black canvas. The artwork must remain confined to the companion's existing vertical area, never appear as a card or rectangular platform, add no text or implied system activity, and never intercept touch or screen-reader focus. Do not recreate it with opaque geometric slabs or additional orbit decorations. Light mode uses a quiet pale-blue underfoot halo instead of placing a black image block on the light canvas. The stage remains static—the companion supplies the meaningful motion. Center the wordmark in the top bar, retain balanced leading/trailing space, and keep the readiness card non-interactive unless a real action is added later. Place "What should Devin build?" directly above one clean charcoal composer; do not wrap the title and composer in an additional outer card. The composer uses one compact multiline prompt area with the existing add, mode, playbook, and send controls aligned along its bottom edge; it grows for entered text but must not reserve an oversized blank block. Do not add extra control pills from visual references. The repository picker uses one grouped surface with search, flat rows, subtle separators, and a checkmark selection state; do not render every repository as a separate floating card. State is `idle` by default, `working` while session creation is pending, and `error` when `composerError` exists. Home remains centered and does not use the horizontal walking track; walking begins in the created session after navigation. Prompt text must not affect pet state or restart animation.

### Session Detail

Place a compact companion in a dedicated transparent track immediately above the session composer. The track is a stable, pointer-transparent foreground overlay at the bottom of the timeline viewport, so the conversation scrolls behind it and it consumes no opaque layout row. It must not push messages into a visible shelf, live inside the composer, intercept touches, cover controls, or paint a background. The scroll content must include enough transparent tail clearance for every response line to scroll fully above the companion and its caption. Session Detail intentionally uses a much smaller companion than Home so timeline content remains primary. Keep its transparent track mounted throughout the session. Render meaningful current activity as small text directly above the sprite inside the same animated wrapper; the caption moves and stops with Devin instead of remaining centered in the viewport. The caption has no chip, border, card, or opaque fill. An optimistic send or any canonical active-work state begins walking immediately at Devin's current position without jumping to another edge. The semantic activity state and caption may distinguish thinking, editing, testing, reading, executing, responding, completion, errors, or actionable blocks, but every active state uses the direction-specific walking frames until work settles. When work ends, stop movement at the current position and transition directly to waiting, success, error, sleeping, or another canonical animation. Passive idle, waiting, and sleeping states remove the caption and its caption-height reservation. Sending immediately dismisses the keyboard so the timeline, track, and composer remain visible. While the keyboard is open, both Cloud and Computer session composers expose a 44-point, VoiceOver-labeled Hide Keyboard action beside the mic and Send controls; dragging the conversation interactively dismisses it as well. Either dismissal path must preserve the complete draft. Use the exhaustive canonical status mapping and canonical status label for accessibility. Keep captions task-focused and do not duplicate long header copy.

While a turn is active, the timeline tail shows a compact activity trail rather than a generic "Devin is working" row. It may retain at most the three most recent distinct activity labels for the current turn, fading superseded entries and marking only the newest entry as active. Entries are observational, not a task checklist: never mark a prior item successful unless the transport explicitly reports success. Computer labels come only from the minimized ACP activity contract (`thinking`, `reading`, `editing`, `executing`, `searching`, `fetching`, or `responding`) and its bounded display label. Cloud labels come only from the current documented session status and, when supplied, a bounded activity reason returned at the validated session boundary. Never infer filenames, commands, tools, plans, or completion from message text; never expose tool inputs/outputs, thoughts, raw paths, identifiers, or private event payloads. Clear the ephemeral trail when the active turn settles or the session changes.
...
- Directional edge-to-edge travel, keyboard dismissal after Send, interactive drag dismissal, and
  the explicit draft-preserving Hide Keyboard action in both session types.
- Text-only companion captions and the bounded, truthful live-activity trail.
- Responsive Home companion-stage geometry, decorative accessibility isolation, and dark/light treatments.
