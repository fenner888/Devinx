...
- Disclaimer is now load-bearing, not boilerplate. In-app (Settings → About), on the App Store listing, AND on the onboarding Welcome screen: _"DevinX is an independent, unofficial client for the Devin API. Not affiliated with, endorsed by, or a product of Cognition AI."_
...
**Public-release scope decision (July 10, 2026):** public distribution waits until users can choose Cloud only, Local only, or both. Development and internal TestFlight builds still ship in phases so each trust boundary is reviewed independently. Existing internal `computer` identifiers remain unchanged for pairing and persistence compatibility.
...
1. **Welcome** — transparent theme-aware product wordmark without a decorative frame or opaque icon tile, value proposition, Cloud/Local and on-device-voice proof points, independent-client disclaimer, and one primary CTA.
2. **What DevinX supports** — create/steer sessions, Cloud or a paired local device, on-device dictation and prompt organization, genuine Code Scan session inspection, and credential-storage boundaries. Do not advertise unsupported file, approval, scan-creation, billing, or integration mutations.
3. **Connection choice** — `Devin Cloud`, `Local`, or `Cloud + Local`, with distinct truthful descriptions. A Local connection uses the user-controlled Connector and never copies Devin credentials to the phone.
4. **Provider setup** — Cloud onboarding uses a `Service user key` only. Personal-token authentication is not presented until that product path is explicitly approved for release. The API key field is secure-entry and paste-first; the org ID and optional attribution ID follow the existing validated contract. Accept both server-issued organization-ID forms observed across Devin accounts (`org-…` and legacy `org_…`), preserve the value exactly, and let authenticated API validation determine validity; never rewrite one separator into the other. Inline help opens a first-party walkthrough covering least-privilege setup, one-time `cog_` key display, organization-ID location, and the optional attribution permission before offering links to Devin and Devin's current [Authentication](https://docs.devin.ai/api-reference/authentication) documentation. Local setup uses the Tailscale-only QR flow and explicit approval on the local device. The assisted setup detects macOS or Windows and installs only a signed, checksummed Connector package for the detected supported platform. Combined mode must make the sequence explicit on both provider screens: `Step 1 of 2 — Connect Devin Cloud`, followed by `Step 2 of 2 — Pair a local device`. Cloud-only and Local-only setup retain their single-provider headings.
5. **Validate** — Cloud performs a live authenticated sessions call before Keychain persistence. Combined mode continues to Local pairing and explains that the Cloud connection is complete; Cloud-only lands on Home. Failures remain specific enough to recover without exposing secrets or remote response bodies.
...
- **Timeline tab:** chronological messages (cursor-paginated, infinite scroll upward), Devin vs user styling, markdown rendering, and code blocks with copy. Sending a message auto-resumes a suspended session. The canonical header status communicates that the session is sleeping; do not repeat a passive wake helper beside the composer. Devin-produced Cloud image and video attachments appear once in a clearly labeled session-artifacts section at the live tail of the timeline because the published attachment contract is session-scoped and does not identify a parent message. When Devin also emits a machine-readable `ATTACHMENT:{...}` transport marker in message text, DevinX removes the raw marker from the transcript and uses its validated HTTPS URL only to associate or recover the corresponding session artifact. Images open in a full-screen viewer. Videos remain unloaded until an explicit user tap, then use native inline/full-screen controls; they never autoplay.
- **Worklog/Output:** PR list (deep-link to GitHub app/web + Devin Review link if present), documented session-output attachments, structured output viewer (pretty JSON), and session insights (trigger on-demand generation, render results). Use the attachment response's validated `source` and `content_type` fields as the primary presentation signals, with a filename-extension fallback only when `content_type` is absent or generic. Only HTTPS URLs may render. Live `app.devin.ai/attachments/{uuid}/{name}` markers are browser URLs, so DevinX converts only that exact validated shape to the documented organization-scoped `GET /v3/organizations/{org_id}/attachments/{uuid}/{name}` endpoint. The bearer credential is sent only to `api.devin.ai`; the API's short-lived presigned redirect supplies storage access. Media bytes then download into the OS-managed cache; credentials never enter component props, query caches, filenames, URLs, logs, error copy, the web-app host, arbitrary hosts, or the presigned storage request. Cached previews are deleted when their renderer unmounts. Non-media files are labeled and opened from the canonical Devin session rather than sending the private URL to an unauthenticated browser. Media failures remain recoverable through an attachment refresh/retry and never expose a URL, query string, credential, or raw response in logs or user-facing errors. Every scrollable detail tab reserves clearance for the fixed composer so the final rows remain reachable.
- **Computer-session boundary:** ACP attachment transfer remains unavailable until its independent permission, transport, size, path, and content-type contract is specified. DevinX must not imply that a Cloud session-artifact renderer enables local file transfer.
...
- [ ] Local model controls come only from the live ACP catalog, and a harmless real session proves the exact selected model (including Adaptive when offered) is confirmed before prompting
...
**Session 8 (Phase 3B, required before public release)** — macOS Local Connection: a user-controlled bridge wrapping the supported ACP subprocess, QR pairing, per-device credentials and permissions, read-only session browsing first, then explicitly authorized message steering. LAN/Tailscale follows localhost validation; public tunnels and relays remain deferred.
