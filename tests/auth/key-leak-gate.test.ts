 ...
 *    OUTSIDE /src/auth, /src/lib/branding.ts, /src/lib/diagnostics.ts, /src/api/devin/schemas.ts
 */
...
describe('key-leak grep gate (§10.1)', () => {
  ...
  it('has no real cog_ keys hardcoded', () => {
    ...
    for (const f of files) {
      ...
      // diagnostics.ts contains the scrubber regex pattern, not a real key.
      if (rel.endsWith('lib/diagnostics.ts')) continue;
  ...
  it('keeps secret-adjacent variable names inside /src/auth or allowlisted files', () => {
    ...
    for (const f of files) {
      ...
      const content = fs.readFileSync(f, 'utf8');
      // Allow the branding keychain key strings and diagnostic scrub patterns.
      if (rel.endsWith('branding.ts') || rel.endsWith('diagnostics.ts')) continue;
