...
const codeScanReferenceSnippetSchema = z
  .object({
...
export const codeScanFindingSchema = z
  .object({
    ...
    code_owners: z.array(z.string()),
    reference_snippets: z.array(codeScanReferenceSnippetSchema),
    ...
    orchestrator_session_id: z.string().nullable(),
...
export const codeScanMetricsSchema = z
  .object({
    scans_count: z.number().int().nonnegative(),
    repos_scanned_count: z.number().int().nonnegative(),
    prs_created_count: z.number().int().nonnegative(),
    prs_open_count: z.number().int().nonnegative(),
    prs_merged_count: z.number().int().nonnegative(),
    prs_closed_count: z.number().int().nonnegative(),
    avg_pr_time_to_merge_seconds: z.number().nonnegative().nullable(),
    avg_pr_open_duration_seconds: z.number().nonnegative().nullable(),
    open_critical_findings_count: z.number().int().nonnegative(),
    open_high_findings_count: z.number().int().nonnegative(),
    open_medium_findings_count: z.number().int().nonnegative(),
    open_low_findings_count: z.number().int().nonnegative(),
  })
  .passthrough();

export const codeScanMetricsRangeSchema = z
  .object({
    timeAfter: z.number().int().nonnegative(),
    timeBefore: z.number().int().nonnegative(),
  })
  .superRefine((range, context) => {
    if (range.timeAfter >= range.timeBefore) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['timeAfter'],
        message: 'timeAfter must be earlier than timeBefore',
      });
    }
    if (range.timeBefore - range.timeAfter > 100 * 24 * 60 * 60) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['timeAfter'],
        message: 'code scan metrics range cannot exceed 100 days',
      });
    }
  });

export const remediateFindingRequestSchema = z.object({
  scanId: z.string().trim().min(1).max(256),
  findingId: z.string().trim().min(1).max(256),
});

export const remediateFindingResponseSchema = z
  .object({
    finding_id: z.string().min(1),
