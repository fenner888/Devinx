...
Connector 0.1.7 fixes a macOS child-process EOF lifecycle defect that could leave stdout and stderr
handlers repeatedly reading closed pipes after the embedded runtime exited. Every EOF, normal exit,
launch failure, explicit stop, and restart now uses one idempotent cleanup path that removes both
handlers and closes all pipe endpoints. The release gate repeatedly exits and restarts real child
processes and requires quiescent idle CPU, stable memory, and zero project-owned disk growth.
...
version: older Connectors keep existing local-session behavior, but users must install 0.1.7 to receive
the latest catalog display metadata.

Existing users update once by quitting Connector, opening the signed 0.1.7 DMG, and replacing the
...
reconnection, or iPhone setup. New Local-mode installs receive 0.1.7 from the same official
