...
| Privacy disclosures and manifests | Implemented; final artifact and human publication required | `PRIVACY.md`, in-app Privacy, `docs/app-privacy-review.md`; dormant notification and crash-reporting SDKs are absent from source, while a superseding IPA must confirm the packaged manifests; App Store answers and stable public policy URL remain unpublished |
...
| Production build/export and artifact integrity | Verified for Build 24 | Build 24 signature, metadata, entitlements, update channel/runtime, `NSFileProtectionComplete`, privacy manifests, and internal upload passed; repeat only if candidate code changes |
